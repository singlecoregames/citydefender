...
export class Hud {
  ...
  render(state: GameState, totalScrap: number): void {
    this.scrapEl.textContent = `⬡ ${totalScrap}`;
    const wave = Math.min(state.director.waveIndex + 1, state.director.totalWaves);
    this.waveEl.textContent = `NIGHT ${state.night} — WAVE ${wave}/${state.director.totalWaves}`;
