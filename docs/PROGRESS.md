...
## 밸런스 현황 (밸런스 시뮬레이터 2차 튜닝 반영)
`npm run sim`(tools/sim/)으로 N1~50 풀런 검증. **시드 4개 전부 클리어,
저숙련(skill 0.5)·greedy 전략도 클리어, 타임아웃 0.** 평균 ~3h08m(시뮬 기준,
Day 35s/회 — 사람 기준 ~4h). **결전 텐션 확보**: N50에서 2~4트라이 + 도시 1~2개
생존으로 클리어되는 클라이맥스 곡선.
- 적 HP: `(1+0.004·(n-1))·1.135^(n-1)` — N4까지 1, N5에 2, N50≈575.
  **초반 제약**: N4가 2HP로 반올림되면(스케일≥1.5) 초반 벽(8연패).
  **결전 제약**: 1.14면 N50이 어느 시드든 불가능, 1.135+lin0은 전 시드 1트라이.
  현재 값이 그 사이의 칼끝 — 바꿀 땐 반드시 `--runs=4`로 분산 확인
- **CHILD_HP_FACTOR 0.6** (신규): 분열/캐리어/보스 자식이 밤 스케일의 60%만
  상속. 자식 풀상속은 후반 밤의 시드 운빨 스파이크(특정 시드만 N50 불가) 원인
- 파워 천장 하향(후반 7배 오버킬 제거): turret_power/speed 14→10렙,
  Overcharge Matrix +40→**+20%**, Cooling Core +25→**+15%**,
  TURRET.levelDamageBonus 0.3→**0.2**, BOSS.hp 55→**45**
- 웨이브: `4 + floor(n/3)`개, 적 수 `(5+w)·1.05^(n-1)` **캡 32/웨이브**
  (캡이 없으면 스폰간격 하한 때문에 N30+ 밤이 물리적으로 10분 초과)
- 보상: rewardGrowth **1.07** (1.13은 트리를 N13에 완매시킴) /
  scrapMul 캡: salvage 8렙·refinery 6렙 /
  **Compound Interest 이자는 그날 수입까지만**(`dawnInterest` 캡) —
  무제한 %는 트리 완매 후 은행을 ×114로 복리 폭증시켰음
- **첫 클리어 Cores**(`firstClearCores()`): N10+부터 첫 클리어마다 ◆1(+N25부터 2)
- Data: comboPerData 20, cap 3 / 보스 처치 Cores = 2 + 밤/10
- 후반 콤보 60~130 유지 — 수동 플레이가 후반에도 살아있음(설계 의도 충족)
