...
- **밸런스 3종** (플레이테스트 논의 반영):
  - **보스 HP 피티**: 보스 밤 연속 패배마다 보스 HP −7% (캡 −30%,
    ...
    N30: 7-8트라이 → ~5트라이.
  - **사령부(HQ) 내구도**: 글로벌 풀 24 + 초당 0.2 자가수리. 죽은 칸 착탄이
    HQ를 깎고(누수 = leak × 죽은칸/(전체−1) — 뚫린 폭 비례), 패배 = HQ 0.
    "1칸 몰빵" 전략의 직접 카운터. 균일 누수(10, 재생 없음)는 보스 밤을 2/3
    생존 상태 45초에 끝내버려 폭 비례로 재설계. 실드 제너레이터는 누수도 흡수.
    HUD: 캐논 위 HQ 바(전선 온전하면 숨김), hqHit 이벤트(셰이크/사운드/파티클),
    위험 비네트 HQ 연동.
  - **필드 공속 1.67배** (DPS 중립): 주기 2.0→1.2s, 펄스 데미지 1→0.6,
    static_charge +0.5→+0.3/lvl (전 레벨 DPS 완전 보존), slowSeconds 0.8→0.6
    (만렙 0.69s 주기의 상시 슬로우 방지 — 슬로우 가동률은 40%→50%로 상승).
    Static Link는 공식상 자동 중립(rate×DPS×주기)이라 무조정.
  - 풀 시뮬 3시드×N120: 동일 진행(N110 벽), 총 실패 61-65 → **48-50**,
    플레이타임 7h → 6h35m. 첫 트라이 난이도 무변화.
